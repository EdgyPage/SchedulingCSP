...
def test_json_is_not_sanitized_but_exports_are():
    ...
    assert tables[0][0]["ID Alias"] == "=id"       # JSON stays raw (frontend escapes)
    assert tables[0][0]["Function"] == "=func"

    csv = ia.results_to_csv_bytes(tables)
    assert b"'=id" in csv and b"'=func" in csv
...
def test_sanitize_cell_leaves_safe_values():
    assert ia._sanitize_cell("Func 1") == "Func 1"
...
def test_export_endpoint_sanitizes(monkeypatch):
    ...
    schedules = [[{"ID Alias": "=cmd|'/C calc'!A1", "Function": "Func 1"}]]
...
def test_validate_inputs_rejects_bad_minimums():
    with pytest.raises(HTTPException):
        _validate_inputs([], ["Func 1"], [float("nan")], 1)      # NaN
    with pytest.raises(HTTPException):
        _validate_inputs([], ["Func 1"], [-1], 1)                # negative
    with pytest.raises(HTTPException):
        _validate_inputs([], ["Func 1", "Func 2"], [1], 1)       # wrong length
...
def test_oversized_roster_and_tasks_rejected(monkeypatch):
    ...
    assert client.post(
        "/api/solve",
        json={"tasks": ["Func 1", "Func 2"], "employees": [], "minimums": [1, 1]},
    ).status_code == 400
